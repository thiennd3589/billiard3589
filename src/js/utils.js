...
function getPositionOfStick(whiteBall, stick, mouse) {
  ...
  let angle = getAngleBetweenMouseAndWhiteBall(mouse.x, mouse.y, whiteBall.x, whiteBall.y);

  let rotateAngle = 0;
  if (angle > 0) {
    rotateAngle = Math.PI / 2 + angle;
  } else if (angle < 0) {
    const beta = Math.PI / 2 - Math.PI - angle;
    rotateAngle = 2 * Math.PI - beta;
  }

  const distance = whiteBall.radius;
...
module.exports = {
  randomIntFromRange,
  randomColor,
  getAngleBetweenMouseAndWhiteBall,
  getDistance,
  resolveCollision,
  getPositionOfStick,
  isMobile
